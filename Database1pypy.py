...
def make_appointment(mykey,mydate, mytime, tutor_id, tutee_id, Class_id):
    with engine.connect() as conn:
        mystring = f"INSERT OR REPLACE INTO APPOINTMENT VALUES('{mykey}','{mydate}', '{mytime}', '{tutor_id}', '{tutee_id}', '{Class_id}')"
        query = text(mystring)
        conn.execute(query)
...
def add_tutee(t_id, name):
    with engine.connect() as conn:
        query=text(f"""Insert INTO Tutee values({t_id}, 1, {name}""")
        Result = conn.execute((query))
...
def add_tutee(t_id, name):
    with engine.connect() as conn:
        query=text(f"""Insert Count(name from Tutee""")
        Result = conn.execute((query))
        return Result.first()._mapping.values()
...
def tutorUpdate():
    if request.method == 'POST':
        ...
        #adding tutor name
        #newTuteeID = request.form['newTuteeID']
...
@app.route('/appointmentdelete', methods=['POST','GET'])
def appointmentDelete():
    if request.method == 'POST':
        appointmentIDDelete = request.form['appointmentID']
        
        #deletion query goes here
        drop_appointment(appointmentIDDelete)
        
        return render_template("appointmentDelete.html")
    else:
        return render_template("appointmentDelete.html")

@app.route('/emailsms', methods=['GET','POST'])
def userInfo():
    if request.method == 'POST':
        smsName = request.form['smsName']
        smsCWID = request.form['smsCWID']
        smsEmail = request.form['smsEmail']
        smsProvider = request.form['radio-select']
    ...
    Thank you for using the OSU LASSO Testing Center!""", smsEmail)
            
        emailAlert("LASSO Appointment Reminder",
                   """Thanks for making an appointment with the LASSO Tutoring Center!        
    Location: 021 Classroom Building            """, smsCWID, smsProvider)
